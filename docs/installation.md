...
XLang has no build step. There's nothing to `npm install`, nothing to
compile. You just load one script, and `<div data-xlang>` blocks on your
page start working.

## Option 1 — Load from a CDN (fastest way to start)

Add this inside your `<head>`, before anything that uses `data-xlang`:
...
repository. It's the quickest way to try XLang, but keep in mind: since
...
```html
<script src="https://cdn.jsdelivr.net/gh/adilson889/Xlang@<commit-hash>/xlang-interpreter.js"></script>
```
...
```html
<div data-xlang>
    <print>XLang is working!</print>
</div>
```

Open the page in a browser. If you see the text "XLang is working!"
appear where the `<div>` used to be, you're set up correctly.
