...
The way you tell the browser "this part is XLang, not plain HTML" is a
single container:

```html
<div data-xlang>
    <!-- everything in here is XLang code -->
</div>
```
...
One rule to always keep in mind: **every XLang tag closes explicitly**.
...
- No build step, no npm install, no compiler — XLang runs directly in the
