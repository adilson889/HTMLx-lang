...
the exact same ideas every larger XLang program uses, just at the smallest
possible scale.

## Step 1 — the container

Every piece of XLang code lives inside a `<div data-xlang>`. Start with an
empty one:
...
`'World'` tells XLang "this is a text string, not a number or a variable
name." Try it without the inner quotes and XLang will look for a variable
...
HTML `<input>` outside the XLang block, and use `<bind>` to connect it:
...
You used three of the most common XLang tags:
